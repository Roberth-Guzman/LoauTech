...
// Procesar el registro de hora de salida
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['registro_salida_id'])) {
    $idElemento = (int) $_POST['registro_salida_id'];
    $stmt = $conn->prepare("UPDATE ingresoelementos SET hora_salida = NOW() WHERE IDingele = ? AND IDper = ? AND hora_salida IS NULL");
    $stmt->bind_param("ii", $idElemento, $usuario_id);
    if ($stmt->execute()) {
        $mensaje = "Hora de salida registrada exitosamente.";
    } else {
        $mensaje = "Error al registrar la hora de salida.";
    }
    $stmt->close();
}

// Obtener los elementos registrados por el usuario
$stmt = $conn->prepare("SELECT IDingele, nombreingele, tipoelemento, descripcioningele, observacioningele, hora_entrada, hora_salida 
...
<head>
    <meta charset="UTF-8">
    <title>Mis Ingresos - LOAUTECH</title>
