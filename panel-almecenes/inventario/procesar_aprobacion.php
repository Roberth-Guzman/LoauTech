...
try {
    ...
    // === ACTUALIZAR ESTADO EN TABLA AUTORIZACION ===
    $sqlUpdateAutorizacion = "UPDATE autorizacion a
        JOIN prestamos p ON p.IDautorizacion = a.IDaut
        SET a.estadoaut = ?
        WHERE p.IDpre = ?";
    $stmtUpdateAut = $conn->prepare($sqlUpdateAutorizacion);
    $stmtUpdateAut->bind_param("si", $nuevoEstado, $idPrestamo);
    if (!$stmtUpdateAut->execute()) {
        throw new Exception("Error al actualizar el estado de autorización: " . $conn->error);
    }
    $stmtUpdateAut->close();
    // === FIN ACTUALIZAR ESTADO EN TABLA AUTORIZACION ===
