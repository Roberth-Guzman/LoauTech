...
try {
    // Consulta para obtener solicitudes pendientes de acción
    // Solo mostramos solicitudes que requieren acción del usuario actual
    $rolUsuario = $_SESSION['usuario']['rol'] ?? '';
    
    // Debug: registrar información del usuario y rol
    logError("Usuario: " . ($_SESSION['usuario']['nombre'] ?? 'No definido') . ", Rol: " . $rolUsuario);
    
    if ($rolUsuario === 'cuentadante') {
        // Cuentadante ve solo solicitudes pendientes iniciales
        $estadosFiltro = "a.estadoaut = 'pendiente' AND a.cargoquienautoriza = 'sistema'";
        logError("Filtro cuentadante: " . $estadosFiltro);
    } elseif ($rolUsuario === 'almacenes') {
        // Almacén ve solo solicitudes aprobadas por cuentadante
        $estadosFiltro = "a.estadoaut = 'pendiente_almacen'";
        logError("Filtro almacenes: " . $estadosFiltro);
    } else {
        // Otros roles ven solicitudes activas (excluyendo rechazadas y aprobadas)
        $estadosFiltro = "a.estadoaut = 'activo' AND a.estadoaut NOT IN ('rechazado', 'aprobado')";
        logError("Filtro otros roles: " . $estadosFiltro);
    }
    ...
    // Debug: registrar la consulta SQL completa
    ...
    // Debug: registrar cuántas solicitudes se encontraron
    logError("Número de solicitudes encontradas: " . count($solicitudes));
...
<body class="bg-gray-100">
        ...
        <nav class="bg-gray-800 text-white shadow-lg">
            <div class="container mx-auto px-4 py-3 flex justify-between items-center">
                ...
                <div class="hidden md:flex space-x-6">
                    ...
                    <a href="historial-aprobaciones.php" class="px-3 py-2 rounded-md text-white hover:bg-gray-700 transition-colors">
                    ...
                    <?php elseif (!empty($solicitudes)): ?>
                        <div class="space-y-4">
                            <?php foreach($solicitudes as $solicitud): ?>
                                <div id="solicitud-<?php echo $solicitud['id_prestamo']; ?>" class="border border-gray-200 rounded-lg overflow-hidden">
                                    <div class="bg-gray-800 text-white px-4 py-3 flex justify-between items-center">
                                        ...
                                        if ($solicitud['estado_autorizacion'] === 'activo') {
                                            $mostrarBotones = true;
                                            $estadoTexto = 'Pendiente';
                                            $estadoClase = 'bg-yellow-100 text-yellow-800';
                                        } elseif ($solicitud['estado_autorizacion'] === 'pendiente_almacen' && $rolUsuario === 'almacenes') {
                                            $mostrarBotones = true;
                                            $estadoTexto = 'Pendiente Almacén';
                                            $estadoClase = 'bg-blue-100 text-blue-800';
                                        } else {
                                            $estadoTexto = ucfirst($solicitud['estado_autorizacion']);
                                            $estadoClase = 'bg-gray-100 text-gray-800';
                                        }
                                        ?>
                                        
                                        <div class="flex items-center gap-3">
                                            <span class="<?php echo $estadoClase; ?> px-2 py-1 rounded text-xs font-medium">
    ...
    function closeModal(modalId) {
      ...
      // Siempre limpia el campo de motivo si es el modal de rechazo
      if (modalId === 'rejectModal') {
        const motivo = document.getElementById('rejectReason');
    ...
    function approveRequest(id) {
      ...
      // Usar AJAX en lugar de form submit para manejar la respuesta
      ...
      .then(data => {
        if (data.success) {
          // Ocultar la solicitud con animación
          const solicitudElement = document.getElementById('solicitud-' + id);
          if (solicitudElement) {
            solicitudElement.style.transition = 'opacity 0.5s ease-out, transform 0.5s ease-out';
      ...
      .finally(() => {
        ...
        // Cierre forzado del modal de aprobación (por si acaso)
        closeModal('approveModal');
      });
    }
    function rejectRequest(id, motivo) {
      ...
      .finally(() => {
        ...
        btn.disabled = false;
        btn.innerHTML = originalText;
        // Cierre forzado del modal de rechazo (por si acaso)
        closeModal('rejectModal');
      });
    }
    
    function checkIfNoMoreRequests() {
      const solicitudesContainer = document.querySelector('.space-y-4');
      const solicitudesRestantes = solicitudesContainer.querySelectorAll('[id^="solicitud-"]');
      
      if (solicitudesRestantes.length === 0) {
        // No quedan solicitudes, mostrar mensaje
        solicitudesContainer.innerHTML = `
          <div class="text-center py-8">
            <i class="fas fa-inbox text-4xl text-gray-400 mb-2"></i>
            <p class="text-gray-500">No hay solicitudes pendientes para mostrar.</p>
            <p class="text-gray-400 text-sm mt-2">Todas las solicitudes han sido procesadas.</p>
          </div>
        `;
      }
    }
