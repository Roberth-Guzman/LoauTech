...
<body class="bg-gray-100">
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 ml-64 overflow-auto">
            ...
            <!-- Mensajes de éxito/error (si se implementa) -->
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4">
                    <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-md flex items-center justify-between" role="alert">
                        <div class="flex items-center">
                            ...
                            <p class="text-sm"><?php echo htmlspecialchars($data['mensaje']); ?></p>
                        </div>
                ...
                <div class="max-w-4xl mx-auto">
                    ...
                    <div class="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
                            ...
                            <div class="flex flex-col md:flex-row items-center md:items-start space-y-6 md:space-y-0 md:space-x-6">
                                ...
                                <div class="flex-shrink-0">
                                    <div class="relative group">
                                        <?php 
                                        $foto = $data['usuario']->foto_ruta ?? null;
                                        if ($foto && file_exists(rtrim(PUBLIC_PATH, '/') . '/' . $foto)) {
                                            echo "<img src='" . BASE_URL . "/" . htmlspecialchars($foto) . "' alt='Foto de perfil' class='h-40 w-40 rounded-full object-cover border-4 border-white shadow-md'>";
                                        } else {
                                        ...
                                        <div class="absolute inset-0 rounded-full flex items-center justify-center bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <div class="flex space-x-2">
                                                <?php if ($foto): ?>
                                                <a href="<?php echo BASE_URL . '/' . htmlspecialchars($foto); ?>" target="_blank" class="h-10 w-10 bg-white bg-opacity-90 rounded-full flex items-center justify-center text-blue-600 hover:bg-white transition-colors" title="Ver foto">
                                                ...
                                                <?php if ($foto): ?>
                                                <a href="<?php echo BASE_URL; ?>/porteria/borrarFoto" onclick="return confirm('¿Seguro que quieres borrar tu foto de perfil?');" class="h-10 w-10 bg-white bg-opacity-90 rounded-full flex items-center justify-center text-red-600 hover:bg-white transition-colors" title="Eliminar foto">
                                ...
                                <div class="flex-1">
                                    <div class="space-y-4">
                                        <div>
                                            <h4 class="text-lg font-medium text-gray-900"><?php echo htmlspecialchars($data['usuario']->nombrecompletoper ?? 'No disponible'); ?></h4>
                                            <p class="text-sm text-gray-500"><?php echo ucfirst(htmlspecialchars($data['usuario']->rol ?? 'portería')); ?></p>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p class="mt-1 text-sm text-gray-900 break-all">
                                                    <?php echo !empty($data['usuario']->correocont) ? htmlspecialchars($data['usuario']->correocont) : '<span class="text-gray-400">No especificado</span>'; ?>
                                                </p>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <p class="mt-1 text-sm text-gray-900">
                                                    <?php echo !empty($data['usuario']->numerocont) ? htmlspecialchars($data['usuario']->numerocont) : '<span class="text-gray-400">No especificado</span>'; ?>
                                                </p>
                                            </div>
                                            ...
                                            <div>
                                                <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">
                                                    <?php echo htmlspecialchars($data['usuario']->tipodocumento ?? 'Documento'); ?>
                                                </p>
                                                <p class="mt-1 text-sm text-gray-900">
                                                    <?php echo !empty($data['usuario']->numerodoc) ? htmlspecialchars($data['usuario']->numerodoc) : '<span class="text-gray-400">No especificado</span>'; ?>
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end">
                            <a href="<?php echo BASE_URL; ?>/porteria/editarPerfil" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
            ...
            <div id="modalSubirFoto" class="fixed z-10 inset-0 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                        <form action="<?php echo BASE_URL; ?>/porteria/subirFoto" method="post" enctype="multipart/form-data">
