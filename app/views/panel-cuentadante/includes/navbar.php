...
<nav class="bg-gray-800 text-white shadow-lg">
    <div class="container mx-auto px-4 py-3 flex justify-between items-center">
        <div class="flex items-center space-x-2">
            <a href="<?= BASE_URL ?>/cuentadante/panelPrincipal" class="flex items-center space-x-2">
                <i class="fas fa-clipboard-check"></i>
                <span class="font-bold">Panel Cuentadante</span>
            </a>
        </div>
        <div class="hidden md:flex space-x-6">
            <?php $active_menu = $data['active_menu'] ?? ''; ?>

            <a href="<?= BASE_URL ?>/cuentadante/panelPrincipal"
               class="px-3 py-2 rounded-md text-white transition-colors 
                      <?= $active_menu == 'peticiones' ? 'bg-blue-700' : 'hover:bg-gray-700' ?>">
                <i class="fas fa-clipboard-list mr-1"></i> Peticiones
            </a>
        </div>
        ...
        <div class="relative ml-4">
            ...
            <div id="user-dropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50">
                <a href="<?= BASE_URL ?>/cuentadante/perfil" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
